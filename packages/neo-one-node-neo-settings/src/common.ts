import { addressToScriptHash, common as clientCommon, crypto, Op, UInt160 } from '@neo-one/client-common';
...
const getGenesisBlock = ({ consensusAddress, messageMagic }: GetGenesisBlockOptions) => {
  const block = new Block({
    header: new Header({
      ...
      index: 0,
      primaryIndex: 0,
      nextConsensus: clientCommon.stringToUInt160(addressToScriptHash('NYmMriQPYAiNxHC7tziV4ABJku5Yqe79N4')), // consensusAddress,
  ...
  console.log(block);
  console.log(block.hashHex);
  console.log(
    crypto.scriptHashToAddress({
      addressVersion: 0x35,
      scriptHash: clientCommon.stringToUInt160(addressToScriptHash('NYmMriQPYAiNxHC7tziV4ABJku5Yqe79N4')),
    }),
  );
